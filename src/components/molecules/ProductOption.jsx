...
const ProductOption = ({product}) => {
    ...
    const handleOnClickOption = (option) => {
        ...
        if(isOptionSelected) {
            setSelectedOptions((prev) => 
                prev.map((prevOption) => prevOption.optionId === option.id 
    ...
    return(
        <Option.Container>
            ...
            {/* 사용자가 선택한 옵션 option */}
            <OptionList items={product.response.options} onClick={handleOnClickOption}/>
            <Option.Delivery>
                <Option.DeliveryTitle>
                    배송방법
                </Option.DeliveryTitle>
                <span>
                    택배배송
                ...
                <div key={`selected-option-key-${option.optionId}`}>
                    <Option.SelectedOption>
                        ...
                        <Option.CounterContainer>
                            ...
                            <span style={{marginRight: '1em', cursor: 'pointer', color: 'red'}} onClick={() => handleOnClickDelete(option.optionId)}>X</span>
                        </Option.CounterContainer>
                    </Option.SelectedOption>
                </div>
            ))}
            <Option.Result>
                <span>총 수량: {calculateTotalQuantity()}개</span><span>총 주문금액: {calculateTotalPrice()}원</span>
            </Option.Result>
            <Option.Purchase>
                ...
                <Button className="talkButton" onClick={() => {
                    if(JSON.parse(getLocalStorage("user")).value) {
                        mutate(selectedOptions.map((el) => {
                            return {
                                optionId: el.optionId,
