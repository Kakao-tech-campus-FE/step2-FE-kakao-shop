...
2. 회원가입 페이지
    - 핵심 기능: 회원가입 정보 저장, 아이디 중복 체크, 개인정보 동의 확인
    - 기능 상세 설명: 아이디 및 비밀번호를 받아 회원가입을 진행하고 회원 정보 및 개인정보 동의 여부를 저장하기. 아이디 중복 체크 하기
    - 인터페이스 요구사항: 회원 정보 입력 및 저장하고, 개인정보 동의를 받아야 한다. 아이디 중복체크 요청을 처리해야한다.
3. 메인 페이지
    - 핵심 기능: 상품 목록 보여주기,
    - 기능 상세 설명: 상품 전체 목록 보여주기
    - 인터페이스 요구사항: 스크롤에 따라 페이지 별로 상풀 목록을 보여주어야 한다.
4. 제품 상세 페이지
    - 핵심 기능: 상품 정보 보여주기, 상품 구매 진
    - 기능 상세 설명: 구매 옵션 선택 후 구매 진행
    - 인터페이스 요구사항: 선택한 옵션 보여주어야한다. 옵션이 선택되지 않았을 경우 오류 메시지를 띄워야 한다.
5. 주문내역 페이지
    - 핵심 기능: 주문한 상품 내역 보여주기
    - 기능 상세 설명: 주문한 상품을 지정된 배송 단계 별로 나타낸다.
    - 인터페이스 요구사항: 주문한 상품을 상품 배송 단계 별로 카테고리 나누고, 가격 및 환불 or 리뷰 쓸 수 있도록 해야한다.
6. 장바구니 페이지
    - 핵심 기능: 장바구니 담은 상품 구매 진행
    - 기능 상세 설명: 장바구니에 담은 상품을 전체 구매 or 삭제 처리(월요일 강의에서 이번 프로젝트는 개별 처리는 하지 않는다고 들었습니다.)
    - 인터페이스 요구사항: 장바구니 등록 상품을 삭제할 수 있고 구매 진행할 수 있어야합니다.
7. 마이페이지
    - 핵심 기능: 회원 정보, 찜한 상품, 최근 본 상품, 쿠폰
    - 기능 상세 설명: 회원 정보 및 찜, 최근 본 상품, 쿠폰 내역 등 상태 관리
    - 인터페이스 요구사항: 내 정보를 보여줘야한다. 계정 설정을 변경할 수 있어야 한다. 최근 본 상품, 찜한 상품, 보유한 쿠폰 등을 보여줘야 한다.

# 디렉토리 구조
- public
- src
    - components
        - atoms // 재사용이 가능한 가장 작은 단위
        - molecules // 한가지 일로 역할을 달성할 수 있는 단위
        - organisms // 기능을 조합하여 페이지의 몇 부분을 구성하는 단위
        - templates // 페이지의 구조(뼈대)를 나타냄
    - pages
    - hooks
    - routes
    - styles
...
- ...
```
>>>>>>> 7fd6e927c3c303d0697da40e73961a01b3db579c
...
카카오 크램폴린 IDE에서 배포를 진행하였습니다.
먼저 IDE에 git의 코드를 pull 해옵니다.(https://github.com/LimSumi/step2-FE-kakao-shop)
main 브랜치를 가져옵니다.
배포를 하면 배포 링크로 기본 url을 바꿔줘야합니다.
```
