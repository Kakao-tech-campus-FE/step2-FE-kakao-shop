...
- 인터페이스 요구사항: 이메일 또는 비밀번호에 들어온 값이 적합하지 않은 경우 적절한 알람을 보냅니다.

2.회원가입 페이지
- 핵심 기능: 회원가입 요청 및 사용자가 입력한 정보 저장
- 기능 상세 설명: 이메일 및 비밀번호, 비밀번호 확인 값을 입력 받아, 이에 대한 상태 처리를 합니다
- 인터페이스 요구사항: 이메일 중복 체크 값과 비밀번호와 비밀번호 확인 값이 적합하지 않은 경우 적절한 알람을 보냅니다.

3.메인 페이지
- 핵심 기능: 사용자들이 Head와 Body부분에 있는 적절한 콘텐츠들로 이동하거나 볼 수 있게 합니다. 
- 기능 상세 설명: 헤더에는 각종 링크를 담고 있는 Nav와 로그인, 장바구니 페이지,검색 페이지로 이동 할 수있는 아이콘을 배치하고, Body부분에 상품 카테고리와 카테고리에 맞는 상품들을 나열 합니다
- 인터페이스 요구사항: Nav안의 링크들에 포커스가 되면 아래에 밑줄과 긁씨 굵기 효과가 나타나고 바디부분에 상품에 마우스가 포커스 되면 상품이 약간 확대 되는 효과가 나타나고, 몇명의 사용자가 구매했는데, 가격, 상품 정보, 배송 정보를 나타냅니다

4.개별 상품 상세 페이지
- 핵심 기능: 사용자들이 상품에 대한 상세 정보와 리뷰, 문의를 할 수있고, 상품 사진 및 가격, 평점, 배송 관련 정보을 확인하고 장바구니에 담을 수 있거나, 결제 페이지로 이동 할 수있게 합니다
- 기능 상세 설명: 상품들은 리스트로 만들어 여러 사진들을 하나씩 선택하여 크게 볼 수 있고, 장바구니에 담으면 바로 장바구니 페이지로 이동 하겠냐고 물어보는 알람 창을 띄웁니다. 상세정보, 리뷰, 문의를 카테고리로 묶어서 하나씩 클릭하여 자세히 볼 수 있게 만듭니다
- 인터페이스 요구사항: 바디 부분에 상품 사진, 상품 정보, 장바구니에 담기, 결제 페이지로 이동하는 아이콘 들을 만들고, 바디의 아래 부분에는 상세, 정보 리뷰, 문의가 카테고리로 묶여 하나씩 클릭하여 볼 수 있게 만들어 줍니다  

5.장바구니 페이지
- 핵심 기능: 본인이 장바구니에 담은 상품들을 확인해 예상 금액을 확인하고 결제 페이지로 이동 할 수 있게 합니다
- 기능 상세 설명: 전체 선택 및 상품 개별로 체크를 통해 본인이 원하는 상품의 개수를 골라 주문 금액과 할인 금액과 전체 주문 예상금액을 계산해 확인 할 수 있게 합니다 
- 인터페이스 요구사항: 고른 상품에 맞게 예상 주문 가격을 전체적으로 변경 해 줍니다 

6.결제 페이지
- 핵심 기능:배송지 정보 및 배송 요청 사항을 저장하고, 주문 상품 정보를 확인하고 결제 금액과 배송비, 할인 금액 확인, 포인트 사용 금액을 확인 하고 일반 결제 금액까지 보여줍니다 그리고 결제 방법을 선택하게끔하고 현금 영수증 발급까지 할 수 있게하고 결제 버튼을 만들어 줍니다
- 기능 상세 설명: 기본 배송지 정보가 있다면, 기본 배송지 정보로 배송지를 설정하고 직접 변경 할 수 도 있게합니다. 결제 금액 확인 창에서 포인트를 사용 할 수 있게 합니다 
- 인터페이스 요구사항: 포인트 사용 금액에 맞게 바로바로 일반 결제 금액에 반영 되게 합니다. 결제 수단 선택은 체크바로 만듭니다, 현금 영수증 발급 신청 선택 여부도 체크바로 만듭니다

7.상품 검색 페이지
- 핵심 기능: 상품을 검색 할 수 있는 검
README.md의 예시 형식입니다. 아래를 참색바가 있고, 각종 키워드 및 카테고리로 이동 할 수 있는 아이콘들을 만듭니다.
- 기능 상세 설명: 최근에 검색했던 키워드들을 볼 수 있게 합니다.
- 인터페이스 요구사항: 페이지 초기 실행시 자동으로 검색 창에 포커스를 줍니다.


8.상품 검색 결과 페이지
- 핵심 기능: 상품을 검색어에 맞게끔 검색하여 검색 결과에 맞는 상품들을 보여줍니다.
- 기능 상세 설명: 검색어와 연관된 키워드들을 볼 수 있게 합니다
- 인터페이스 요구사항: 한 행에 2개의 상품씩 밑으로 내리면 계속 상품들이 나오게끔 만듭니다
